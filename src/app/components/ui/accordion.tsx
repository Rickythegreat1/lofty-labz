...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    ...
    >
      {children}
      <ChevronDown className="h-4 w-4 shrink-0 text-[var(--purple-300)] transition-transform duration-200" />
    </AccordionPrimitive.Trigger>
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <div className={cn('pb-4 pt-0 text-[var(--lavender-200)]', className)}>{children}</div>
  </AccordionPrimitive.Content>
));
